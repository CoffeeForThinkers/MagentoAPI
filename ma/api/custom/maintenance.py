...
class MaintenanceApi(ma.api.base_class.Api):
    def __init__(self):
        self.__uh = ma.config.magento.UTILITY_HASH
        self.__mmu = ma.config.magento.MAGENTO_MAINTENANCE_URL

        self.__mct =  ma.config.magento.MAGENTO_CACHE_TYPES
        self.__mit = ma.config.magento.MAGENTO_INDEX_TYPES
    ...
    def refresh_cache(self, cache_types=None):
        if self.__uh is None:
            raise ma.exceptions.MissingUtilityHashError("cache")
        ...
        for (k, desc) in entries:
            url =  self.__mmu['cache'].format(k, self.__uh)
            r = requests.get(url)
    ...
    def reindex(self, index_types=None):
        if self.__uh is None:
            raise ma.exceptions.MissingUtilityHashError("reindex")
        ...
        for (k, desc) in entries:
            url =  self.__mmu['index'].format(k, self.__uh)
            r = requests.get(url)
